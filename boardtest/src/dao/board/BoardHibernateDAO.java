...
public class BoardHibernateDAO implements BoardDAOImpl {
	...
	public void insert(BoardModel boardModel) {
		Session session = this.sessionFactory.openSession();
		//INSERT,UPDATE,DELETE 사용시 트랜잭션을 얻어온다.
	...
	public void update(BoardModel boardModel) {
		...
		try {
			...
			//변경된 내용이 기존의 BoardModel에 설정한 것만으로도 UPDATE가 이루어진다.
			//board.xml에서 <id/>와 num값이 매핑되어 수정
			//어떻게보면 이부분이 ORM기술을 뚜렷히 볼 수 있는 부분
			//필히 기존의 앖을 조회해 와서 변경된 내용을 설정해야된다.
			transaction.commit();
		} catch (Exception e) {
			transaction.rollback();
			e.printStackTrace();
	...
	public void delete(BoardModel boardModel) {
		...
		Transaction transaction = session.beginTransaction();
		try {
			//delete 메소드 실행(boardModel에는 num값이 들어가있어야함)
			//board.xml에서 <id/>와 num값이 매핑되어 삭제
